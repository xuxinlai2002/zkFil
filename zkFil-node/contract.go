...
var zkFilExchangeClient *zkFilExchange

//ConnectToProvider connects to contract
func ConnectToProvider(key *keystore.Key, zkFilEXAddr string, Log ILogger) error {
	...
	zkFilEXContractAddr := common.HexToAddress(zkFilEXAddr)
	pClient, err := NewzkFilExchange(zkFilEXContractAddr, client)
	if err != nil {
		return err
	}
	GAdminAuth = auth
	zkFilExchangeClient = pClient
	Log.Infof("connect to contract...contract addr=%v", zkFilEXAddr)
	return nil
}
...
func publishPlainDataToContract(b Bulletin, value int64) (string, error) {
	...
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.Publish(GAdminAuth, size, s, n, mklroot, big.NewInt(0), big.NewInt(0))
	if err != nil {
...
func publishTableDataToContract(b Bulletin, value int64) (string, error) {
	...
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.Publish(GAdminAuth, 0, s, n, mklroot, digest, big.NewInt(1))
	if err != nil {
		return "", fmt.Errorf("failed to Publish: %v", err)
	}
	return ctx.Hash().Hex(), nil
...
func closeDataAtContract(bltByte [32]byte) (string, bool, error) {
	...
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.UnPublish(GAdminAuth, bltByte)
	if err != nil {
...
func readDataStatusAtContract(bltByte [32]byte) (string, error) {
	...
	bulletins, err := zkFilExchangeClient.zkFilExchangeCaller.Bulletins(&bind.CallOpts{}, bltByte)
	if err != nil {
...
func withdrawAETHFromContract(bltByte [32]byte, Log ILogger) (string, error) {
	t := time.Now()
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.WithdrawA(GAdminAuth, bltByte)
	if err != nil {
...
func BobDeposit(value int64, AliceAddr string) (string, error) {
	...
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.BobDeposit(GAdminAuth, common.HexToAddress(AliceAddr))
	if err != nil {
...
func BobUnDeposit(AliceAddr string) (string, error) {
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.BobUnDeposit(GAdminAuth, common.HexToAddress(AliceAddr))
	if err != nil {
...
func BobWithdrawETHFromContract(AliceAddr string) (string, bool, error) {
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.WithdrawB(GAdminAuth, common.HexToAddress(AliceAddr))
	if err != nil {
...
func submitScrtForComplaint(tx Transaction, receiptSign []byte, Log ILogger) (string, error) {
	...
	//func (_zkFilExchange *zkFilExchangeTransactor) SubmitProofComplaint(opts *bind.TransactOpts, _seed0 [32]byte, _sid *big.Int,
	...
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.SubmitProofComplaint(GAdminAuth, *byte32(seedByte), sessionInt,
...
func readScrtForComplaint(sessionID string, AliceAddr string, BobAddr string, Log ILogger) (ComplaintSecret, error) {
	...
	for i := 0; i < 20; i++ {
		...
		t := time.Now()
		records, err := zkFilExchangeClient.zkFilExchangeCaller.GetRecordComplaint(&bind.CallOpts{}, common.HexToAddress(AliceAddr), common.HexToAddress(BobAddr), sessionInt)
		if err != nil {
...
func claimToContractForComplaint(sessionID string, bulletin Bulletin, claimFile string, AliceAddr string, Log ILogger) (string, error) {
	...
	t := time.Now()
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.ClaimComplaint(GAdminAuth, common.HexToAddress(AliceAddr), sessionInt,
...
func settleDealForComplaint(sessionID string, AliceAddr string, BobAddr string) (string, error) {
	...
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.SettleComplaintDeal(GAdminAuth, common.HexToAddress(AliceAddr), common.HexToAddress(BobAddr), sessionInt)
	if err != nil {
...
func submitScrtForAtomicSwap(tx Transaction, receiptSign []byte, Log ILogger) (string, error) {
	...
	_addrs[0] = common.HexToAddress(tx.BobAddr)
	_addrs[1] = common.HexToAddress(tx.AliceAddr)
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.SubmitProofAtomicSwap(GAdminAuth, *byte32(seedByte), _sCnt, sessionInt,
...
func readScrtForAtomicSwap(sessionID string, AliceAddr string, BobAddr string, Log ILogger) (AtomicSwapSecret, error) {
	...
	for i := 0; i < 20; i++ {
		...
		t := time.Now()
		records, err := zkFilExchangeClient.zkFilExchangeCaller.GetRecordAtomicSwap(&bind.CallOpts{}, common.HexToAddress(AliceAddr), common.HexToAddress(BobAddr), sessionInt)
		if err != nil {
			Log.Warnf("Failed to read session record: %v", err)
			return secret, errors.New("Failed to read session record")
		}
...
func submitScrtForAtomicSwapVc(tx Transaction, receiptSign []byte, Log ILogger) (string, error) {
	...
	_addrs[0] = common.HexToAddress(tx.BobAddr)
	_addrs[1] = common.HexToAddress(tx.AliceAddr)
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.SubmitProofAtomicSwapVC(GAdminAuth, seed0Int, randInt,
...
func readScrtForAtomicSwapVc(sessionID string, AliceAddr string, BobAddr string, Log ILogger) (AtomicSwapVcSecret, error) {
	...
	for i := 0; i < 20; i++ {
		...
		t := time.Now()
		records, err := zkFilExchangeClient.zkFilExchangeCaller.GetRecordAtomicSwapVC(&bind.CallOpts{}, common.HexToAddress(AliceAddr), common.HexToAddress(BobAddr), sessionInt)
		if err != nil {
			Log.Warnf("Failed to read session record: %v", err)
			return secret, errors.New("Failed to read session record")
		}
...
func submitScrtForVRFQ(tx Transaction, receiptSign []byte, Log ILogger) (string, error) {
	...
	_addrs[0] = common.HexToAddress(tx.BobAddr)
	_addrs[1] = common.HexToAddress(tx.AliceAddr)
	ctx, err := zkFilExchangeClient.zkFilExchangeTransactor.SubmitProofVRF(GAdminAuth, srInt, sessionInt,
...
func readScrtForVRFQ(sessionID string, AliceAddr string, BobAddr string, Log ILogger) (VRFSecret, error) {
	...
	for i := 0; i < 20; i++ {
		...
		t := time.Now()
		records, err := zkFilExchangeClient.zkFilExchangeCaller.GetRecordVRF(&bind.CallOpts{}, common.HexToAddress(AliceAddr), common.HexToAddress(BobAddr), sessionInt)
		if err != nil {
			Log.Warnf("Failed to read session record: %v", err)
			return secret, errors.New("Failed to read session record")
		}
...
func calcuDeposit(AliceAddr string, BobAddr string, value int64) (bool, error) {
	dpst, err := zkFilExchangeClient.zkFilExchangeCaller.BobDeposits(&bind.CallOpts{}, common.HexToAddress(BobAddr), common.HexToAddress(AliceAddr))
	if err != nil {
